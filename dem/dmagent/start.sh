...
CMD_LINE="\"${JAVA_EXE_FULL_PATH}\" $JAVA_MEM_OPTS -cp \"${AGENT_LIB}/*\" -Dnoconsole=$DAEMON_FLAG -Ddameng.log.file=\"${AGENT_HOME}/log4j.xml\" -Dagent.home=\"${AGENT_HOME}\" -Dagent.pid.file=\"${MY_PID_FILE}\" ${CLIENT_CLASS} $OTHER_ARGS"
